...
class Lista {
  ...
  public void swap(int i, int j) {
    ...
    Presidente []copia = array;
    public void radix_sort( )  
    {  
        Presidente bucket[][] = new Presidente[n][n];  
	...
	int i, j, k, remainder, NOP=0, divisor=1, larger, pass;  
        ...
        for(pass=0; pass<NOP;comp++, pass++) // Initialize the buckets  
        {  
            for(i=0;i<n;comp++,i++)  
            bucket_count[i]=0;  
            for(i=0;i<n;comp++,i++)  
            {  
                ...
                remainder = (array[i].getId()/divisor)%10;  
		...
		bucket_count[remainder] += 1;  
            ...
            for(k=0;k<n;comp++,k++)  
            {  
                for(j=0;j<bucket_count[k];comp++,j++)  
                {  
                    array[i] = bucket[k][j];
		    mov++;  
                    i++;  
...
//end sets------------------------------------------------------
  ...
  public static void main ( String [] args )throws Exception{
    ...
    Arq log = new Arq( );
      log.openWrite ("608507_radixsort.txt");
