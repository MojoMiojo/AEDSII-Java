...
class Lista {
	...
	*/
	public void inserirFinal( Presidente elemento ) {
	...
	public boolean pesquisarLista( String elemento ){
		boolean retorno = false;
		for (Celula i = primeiro.prox; i != null; i = i.prox) {
			if(i.elemento.getNome().equals(elemento)){
				retorno = true;
				i = ultimo;
			}
		}
...
class HashIndiretoLista {
	...
	boolean pesquisar(String elemento){
		...
		for(int i = 0; resp == false && i < tamanho; i++){
			resp = tabela[i].pesquisarLista( elemento );
		}//€nd for
	...
	public void inserirInicio (Presidente elemento){
		int pos = h(elemento);
		tabela[pos].inserirFinal( elemento );
	}
...
public class Presidente{
	...
	public static void main ( String [] args )throws Exception{
		HashIndiretoLista av = new HashIndiretoLista( );
		...
		Arq log = new Arq( );
		log.openWrite ("608507_hashRehash.txt");
