...
class Lista {
   ...
   public void mostrar( ){
      for(int i = 0; i < n; i++){
        array[i].imprimir( );
	//MyIO.println(array[i].getIdade());//
      }//end for
    //MyIO.println( array[4].getNome( ) );
   }//end mostrar( )

   public Presidente getMaior() {

     Presidente maior = array[0];

	for (int i = 0; i < n; i++) {
           if(maior.getIdade( ) < array[i].getIdade( ) ){
              maior = array[i];
           }
	}
     return maior;	
   ...
   public void countingsort() {
      Presidente br = new Presidente( );
      LocalDateTime br1,br2;
      //Array para contar o numero de ocorrencias de cada elemento
      int[] count = new int[( getMaior().getIdade( ) + 1) ];
      Presidente[] ordenado = new Presidente[ n ];

      //Inicializar cada posicao do array de contagem 
      //for (int i = 0; i < count.length; count[i] = br, i++);

      //Agora, o count[i] contem o numero de elemento iguais a i
      for (int i = 0; i < n; count[array[i].getIdade( )]++, comp++, i++);

      //Agora, o count[i] contem o numero de elemento menores ou iguais a i
      for(int i = 1; i < count.length; count[i] += count[i-1],comp++,  i++);

      //Ordenando
      for(int i = n-1; i >= 0; ordenado[count[array[i].getIdade()]-1] = array[i], count[array[i].getIdade()]--, comp++, mov++, i--);

      //Copiando para o array original
      for(int i = 0; i < n; array[i] = ordenado[i], comp++, mov++, i++);

      //Desempate de Idades      
      for( int i = 0; i < n-1; i++){
	      comp++;
	if( array[i].getIdade( ) == array[i+1].getIdade() ){
	comp++;
	  br1  = array[i].getDataNascimento();
	  br2 = array[i+1].getDataNascimento();
	  if( br1.compareTo(br2) < 0 ){
	    comp++;
	    mov++;
...
//end sets------------------------------------------------------
  ...
  public static void main ( String [] args )throws Exception{
    ...
    Arq log = new Arq( );
      log.openWrite ("608507_countingsort.txt");
