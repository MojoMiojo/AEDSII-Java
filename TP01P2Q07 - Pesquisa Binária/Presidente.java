...
class Lista {
  ...
  public void swap(int i, int j) {
    ...
    public String acento(String str){
       
          // str = str.replaceAll( ""+(char)225, "a");//á
          //str = str.replaceAll( ""+(char)243, "o");//ó
          // str = str.replaceAll( ""+(char)237, "i");//í
           str = str.replaceAll( ""+(char)227, "a");//ã
          //str = str.replaceAll( ""+(char)233, "e");//é
	  //str = str.replaceAll( ""+(char)250, "u");//ú
  ...
  /**
   ...
   public void selecao( ) {
      ...
      for (int i = 0; i < (n - 1); i++) {
         ...
         for (int j = (i + 1); j < n; j++){
            a = array[menor].getNome( ).charAt(0);
	    b = array[j].getNome( ).charAt(0);
		...
		menor = j;
	      ...
	      a = array[menor].getNome( ).charAt( 1 );
	      b = array[j].getNome( ).charAt( 1 );
	      	
      	        if( a > b ){
		    menor = j;
		}

	    }//else se Presidente de letras iniciais igual
	 }//end for
   ...
   boolean pesquisaBinaria( String str, int inseridos ){
    boolean resp = false;
    int esq = 0,
    dir = inseridos,
    meio = 0;
    ...
    while( esq <= dir ){
      meio = ( esq + dir ) / 2;
      comp++;
      if ( acento( array[ meio ].getNome( ) ).compareTo( str ) == 0  ){
        comp++;
	resp = true;
        esq = dir + 1;
      }//end if meio
      else if ( acento( array[ meio ].getNome( )).compareTo( str ) > 0 ){
        comp++;
	dir = meio - 1;
      }//end else if
      else{
  ...
  public static void main ( String [] args )throws Exception{
    ...
    for(int n = 0; n < i-1; n++ ){
     ...
     if( n == 106){
      ...
      if( lista.pesquisaBinaria( leituras[n], j ) ){
        MyIO.println("SIM");
      }else{
        MyIO.println("N�O");
      }//end else
     }//end else debug linha
    }//end for
    
    MyIO.println("");
    //lista.mostrar( );
    Arq log = new Arq( );
    log.openWrite ("608507_binaria.txt");
    log.print("608507\t" + ( System.currentTimeMillis( ) - start )+"\t" + lista.getComp( ));
    log.close( );
