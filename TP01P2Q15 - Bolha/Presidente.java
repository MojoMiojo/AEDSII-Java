...
class Lista {
   ...
   public void mostrar( ){
      for(int i = 0; i < n; i++){
        array[i].imprimir( );
	//MyIO.println( array[i].getLocalNascimento( ));
      }//end for
   }//end mostrar( )
  ...
  public void swap(int i, int j) {
	...
	public void bubbleSort( ){
		for( int i = 0; i < n; comp++, i++){
			for (int j = 0; j < n-1; comp++, j++){

				if(  array[j].getLocalNascimento( ).compareTo( 
				     array[j+1].getLocalNascimento( )) > 0 ){
					swap (j,j+1);comp++;
				}//end else if
	      			
				if( array[j].getLocalNascimento( ).equals( 
				    array[j+1].getLocalNascimento( )) ){
					comp++;
				    if( array[j].getIdade( ) > array[j+1].getIdade( ) ){ 
				       comp++;   swap( j, j+1 ); 
				    }
    				}//end if Locais iguais	
			}//end for j
		}//end for i
	}//end bubbleSort( )
...
//end sets------------------------------------------------------
  ...
  public static void main ( String [] args )throws Exception{
    ...
    Arq log = new Arq( );
      log.openWrite ("608507_bolha.txt");
