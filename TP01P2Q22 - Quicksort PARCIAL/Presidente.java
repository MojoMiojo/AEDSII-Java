...
class Lista {
   ...
   public void quicksort( int esq, int dir ){
    LocalDateTime br,br2,pivo;
    int i = esq, j = dir;
    pivo = array[(esq+dir)/2].getInicioMandato( );

    while( i <= j ){
	    comp++;
	    br = array[i].getInicioMandato( );
	    br2= array[j].getInicioMandato( );
	    while ( br.compareTo( pivo ) < 0 ){ 
		    comp++; 
		    i++; 
		    br = array[i].getInicioMandato();
	    }//end while i
	    while ( br2.compareTo( pivo ) > 0 ){ 
		    comp++; 
		    j--; 
		    br2 = array[j].getInicioMandato();
	    }//end while j
...
//end sets------------------------------------------------------
  ...
  public static void main ( String [] args )throws Exception{
    ...
    lista.quicksort( 0, j-1 );
    MyIO.setCharset("ISO_8859-1");
    lista.mostrar( );
    MyIO.println("");

    Arq log = new Arq( );
      log.openWrite ("608507_quicksort.txt");
